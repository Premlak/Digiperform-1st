...
if (isset($_SESSION['handler'])) {
    switch($_SESSION['handler']){
            ...
            case "exams":
                ...
                break;
            case "test_series":
                header('Location: ./test_series/index.php');
                exit;
                break;
            case "counseling":
                header('Location: ./counseling/index.php');
                exit;
                break;
            case "university":
...
if (isset($_POST['submit'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM subadmin WHERE email = :id AND password = :password');
    $stmt->execute(['id' => $id, 'password' => $password]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($user) {
        $_SESSION['SubAdmin'] = $user['handler'];
        switch($user['handler']){
            case "courses":
...
<head>
    ...
    <title>Sub Admin Access</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 flex justify-center items-center h-screen">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-sm">
        <h2 class="text-2xl font-bold text-center text-gray-700 mb-6">Sub Admin Login</h2>
        <?php if (isset($error)): ?>
