            ...
            <div class="flex flex-col md:flex-row justify-between items-center">
                ...
                <div class="mt-4 md:mt-0 flex items-center space-x-4">
                    <div class="bg-white/20 backdrop-blur-sm rounded-lg px-3 py-2">
                        <p class="text-sm font-medium">Total Universities: <span class="font-bold"><?= count($universities) ?></span></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function toggleDetails(id, button, index) {
            ...
            if (openDetailsId && openDetailsId !== id) {
                const prevElement = document.getElementById(openDetailsId);
                if (prevElement) {
                    prevElement.classList.add('hidden');
                    const prevButton = prevElement.previousElementSibling.querySelector('button');
                    if (prevButton) {
